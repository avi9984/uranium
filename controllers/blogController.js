...
const createBlog = async function (req, res) {
  try {
    ...
    if (!availableAuthorId) {
      return res
        .status(404)
        .send({ status: false, msg: "Author is not available" });
    }
  ...
  try {
    ...
    if (Object.keys(query).length != 0) {
      ...
      if (getByQuery.length == 0) {
        return res
          .status(404)
          .send({ status: false, msg: "No blogs found by filter" });
      }
    }
  } catch (error) {
...
const deleteBlogs = async function (req, res) {
  try {
    ...
    const blogDeleted = await blogModel.findByIdAndUpdate(
      { _id: blogId },
      { isDeleted: true, deletedAt: Date.now() }
    );
    return res
      .status(200)
      .send({ status: true, msg: " Blog is deleted succesfully" });
  } catch (err) {
    res.status(500).send({ status: false, msg: err.message });
  }
};
...
const deleteByQuery = async function (req, res) {
  try {
    ...
    if (query) {
      ...
      return res
        .status(200)
        .send({ status: true, msg: "Blogs are successfully deleted" });
    }
  } catch (err) {
